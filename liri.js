...
function getTweets() {
    ...
    client.get('statuses/user_timeline', params, function(respError, tweets, response) {
        ...
        errorFunction(respError);
         ...
         // ======= STILL WORKING ON FIXING LOGGING (to be continued) ======

        // fs.appendFile("log.txt", "-----Tweets Log Entry Start-----\n\nProcessed at: \n" + Date() + "\n\n" + "terminal commands: \n" + process.argv + "\n\n" + "Data Output: \n\n", errorFunctionStart);

        console.log("\n-------------- Diana's Latest Tweets --------------\n");

        for (i = 0; i < tweets.length; i++) {
            console.log(i + 1 + ". Tweet: ", tweets[i].text);

            // For alignment once the number of the tweet is 10 or higher
            if (i + 1 > 9) {
...
function searchMovie(searchValue) {
    ...
    request(queryUrl, function(respError, response, body) {
        ...
        if (JSON.parse(body).Error == 'Movie not found!' ) {
            ...
            // fs.appendFile("log.txt", "Sorry, I could not find any movies that matched the title " + searchValue + ". Please check your spelling and try again.\n\n-----OMDB Log Entry End-----\n\n", errorFunctionEnd());
